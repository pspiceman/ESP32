// C3_SCD_LED_AP_Reset_Table_Graph_Mem.ino
// ESP32-C3 + SCD4x + HiveMQ 데모 (메모리 저장 + AP 포털 + LED 이력)
...
// ---------- 핀 정의 ----------
const int LED_PIN     = 10;  // 통신/상태 LED (Active HIGH)
...
void loop() {
  ...
  if (apMode) {
    ...
    if (nowBlink - lastBlinkMs >= 100) {
      ...
      digitalWrite(LED_PIN, apBlinkState ? HIGH : LOW);
...
void initScd4x() {
  Serial.println("SCD4x 초기화 시작");

  scd4x.begin(Wire, 0x62);

  int16_t error = scd4x.stopPeriodicMeasurement();
  if (error) {
    Serial.print("StopPeriodicMeasurement 에러: ");
  ...
  if (error) {
    Serial.print("StartPeriodicMeasurement 에러: ");
    Serial.println(error);
    scd4xStarted = false;
  } else {
    scd4xStarted = true;
...
bool readSensors(float &temp, float &hum, float &co2, int &solar) {
  ...
  int16_t error = scd4x.readMeasurement(co2Raw, tempRaw, humRaw);
  if (error) {
    Serial.print("SCD4x readMeasurement 에러: ");
    Serial.println(error);
    return false;
  }
  if (co2Raw == 0) {
    Serial.println("SCD4x 데이터 아직 준비 안 됨 (co2Raw=0)");
    return false;
  }
...
void ledBlinkOnce(uint16_t ms) {
  digitalWrite(LED_PIN, HIGH);
  delay(ms);
  digitalWrite(LED_PIN, ledLogicalState ? HIGH : LOW);
}
