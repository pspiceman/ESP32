...
const uint16_t RX_PIN = 13;
const uint16_t TX_PIN = 12;

// ===== BLE action queue =====
enum BleActKind : uint8_t { BLE_ACT_KEY=1, BLE_ACT_MEDIA=2 };
struct BleAct { uint8_t kind; uint8_t key; uint8_t b0; uint8_t b1; uint16_t holdMs; };
static const uint8_t BLE_ACT_Q_LEN = 12;
...
// ===== IR / RF codes =====
static const uint16_t RAW_POWER2[67] = {9068,4468,608,530,604,1670,604,532,604,1670,576,554,608,1666,606,532,606,1670,602,1662,610,1664,610,530,604,530,606,1644,628,1666,608,532,604,528,606,1664,610,528,608,1664,608,532,604,526,608,526,610,1664,606,526,610,1666,604,1662,610,1662,608,1662,610,1662,612,1664,606,1662,608,1670,584};
static const uint16_t RAW_VOLM2[67]  = {9068,4466,612,526,610,1666,608,528,608,1664,608,528,610,1666,608,530,602,1666,608,1662,612,1664,608,536,600,540,596,1666,608,1666,608,526,610,526,610,1662,610,526,612,1662,610,532,604,1662,612,528,606,528,608,528,608,1662,610,1664,608,1666,608,1666,608,1644,630,1664,608,1648,626,1664,610};
static const uint16_t RAW_VOLP2[67]  = {9066,4448,628,528,608,1662,612,528,606,1666,608,528,608,1662,610,532,604,1664,608,1666,612,1662,608,526,608,510,626,1662,612,1664,608,528,606,528,610,1662,610,528,606,528,610,1668,602,528,608,528,608,532,604,536,598,1670,606,1662,612,1666,606,1664,610,1644,630,1666,606,1666,608,1664,610};
struct IRCode { const char* name; decode_type_t protocol; uint64_t value; uint16_t bits; const uint16_t* raw; uint16_t raw_len; uint16_t khz; };
IRCode irCodes[] = {
...
struct RFCode { const char* name; unsigned long value; uint8_t bits; uint8_t protocol; };
RFCode rfCodes[] = {
  {"DOOR", 12427912, 24, 1}, {"LIGHT", 8698436, 24, 1}, {"SPK1", 15256641, 24, 1}, {"SPK2", 15256642, 24, 1},
};
...
const unsigned long MQTT_RETRY_INTERVAL_MS = 4000;
...
StringQueue<10> bleCmdQ;
StringQueue<8> irCmdQ;
StringQueue<8> rfCmdQ;

uint16_t hexToU16(String h){ h.replace("0x",""); h.replace("0X",""); h.trim(); return (uint16_t)strtoul(h.c_str(), nullptr, 16); }
...
void publish433Status(){
  ...
  if(mqtt.connected()) mqtt.publish(TOPIC_433_STATUS, json.c_str(), true);
}

void publishIrStatus(const String& msg){ if(mqtt.connected()) mqtt.publish(TOPIC_IR_STATUS, msg.c_str(), false); }
void blinkNotify(uint16_t ms){ digitalWrite(NOTIFY_LED_PIN, HIGH); notifyOffAt = millis() + ms; notifyActive = true; }
void serviceNotify(){ if(notifyActive && (int32_t)(millis() - notifyOffAt) >= 0){ digitalWrite(NOTIFY_LED_PIN, LOW); notifyActive = false; } }

void publishBleStatus(bool force){
  if(!force && (millis() - lastBleStatusMs < 1000)) return;
  lastBleStatusMs = millis();
  const unsigned long nowMs = millis();
  bool real = bleKeyboard.isConnected();
  if(real) bleLastOkMs = nowMs;
  bool eff = real;
  if(!eff && bleLastOkMs > 0 && (nowMs - bleLastOkMs) < BLE_DISCONNECT_GRACE_MS) eff = true;

  if(force || eff != bleStableState){
    bleStableState = eff;
    ...
    Serial.printf("[BLE] real=%d eff=%d lastOkAgo=%lu ms\n", real?1:0, eff?1:0, (bleLastOkMs==0)?0UL:(unsigned long)(nowMs - bleLastOkMs));
  }

  if(mqtt.connected() && (force || millis() - lastBleHeartbeatMs >= 2000)){
    lastBleHeartbeatMs = millis();
    uint32_t ts = (uint32_t)(millis()/1000);
    String payload = String("{\"ble\":") + (bleStableState?"true":"false") + ",\"ts\":" + ts + "}";
    mqtt.publish(TOPIC_MIBOX_STATUS, payload.c_str(), true);
  }
}

void publishAllStatusNow(){
  publish433Status();
  publishIrStatus("online");
  publishBleStatus(true);
}
...
void startMQTTIfNeeded(){
  ...
  bool ok = mqtt.connect(cid.c_str(), TOPIC_MIBOX_STATUS, 0, true, "{\"ble\":false,\"ts\":0}");
  if(ok){
    Serial.println("OK");
    ...
    publishAllStatusNow();
    Serial.println("[MQTT] subscribed + status republished");
  } else {
...
void serviceBleActions(){
  static bool active=false; static BleAct cur{}; static uint8_t stage=0; static uint32_t t0=0;
  uint32_t now = millis();
  if(!active){ if(!bleDequeue(cur)) return; active=true; stage=0; t0=now; }
  if(stage==0){
    if(cur.kind==BLE_ACT_KEY) bleKeyboard.press(cur.key);
    else { MediaKeyReport r = { cur.b0, cur.b1 }; bleKeyboard.press(r); }
    t0 = now; stage = 1; return;
  }
  if(stage==1){
    if((uint32_t)(now - t0) < (uint32_t)cur.holdMs) return;
    if(cur.kind==BLE_ACT_KEY) bleKeyboard.release(cur.key);
    else { MediaKeyReport r = { cur.b0, cur.b1 }; bleKeyboard.release(r); }
    t0 = now; stage = 2; return;
  }
...
void handleBleCmd(const String& src){
  String msg = src; msg.trim();
  ...
  else if (msg=="volup")   tapMediaRaw(0x00, 0x80);
  else if (msg=="voldown") tapMediaRaw(0x00, 0x40);
  else if (msg=="mute")    tapMediaRaw(0x00, 0x04);
  else if (msg=="reset")   ESP.restart();
  else if (msg.startsWith("MB:")){
    uint16_t v = hexToU16(msg.substring(3));
    uint8_t msb = (v>>8)&0xFF; uint8_t lsb = v & 0xFF;
    tapMediaRaw(lsb, msb);
  } else {
...
void handleIrCmd(const String& src){
  String msg = src; msg.trim();
  Serial.printf("[IR][RUN] %s\n", msg.c_str());
  StaticJsonDocument<256> doc;
  if(deserializeJson(doc, msg)){ publishIrStatus("JSON parse failed"); return; }
  const char* cmd = doc["cmd"];
  if(!cmd){ publishIrStatus("Invalid JSON: missing cmd"); return; }
  bool ok=false;
  for(int i=0;i<IR_COUNT;i++){
    if(strcmp(irCodes[i].name, cmd)==0){
      if(irCodes[i].raw && irCodes[i].raw_len > 0) irsend.sendRaw(irCodes[i].raw, irCodes[i].raw_len, irCodes[i].khz);
      else irsend.send(irCodes[i].protocol, irCodes[i].value, irCodes[i].bits);
      ok=true; break;
    }
  }
  if(ok) blinkNotify();
...
void handleRfCmd(const String& src){
  String msg = src; msg.trim();
  publish433Log("CMD RX: " + msg);
  if(msg.equalsIgnoreCase("reset")){
    publish433Log("RESET by MQTT"); delay(250); ESP.restart(); return;
  }
  bool ok=false;
  for(int i=0;i<RF_COUNT;i++){
    if(msg.equalsIgnoreCase(rfCodes[i].name)){
      ...
      publish433Log("RF TX " + msg + " [value:" + String(rfCodes[i].value) + ", bits:" + String(rfCodes[i].bits) + ", proto:" + String(rfCodes[i].protocol) + "]");
      ok=true; blinkNotify(); break;
    }
  }
...
void serviceCmdQueues(){
  String msg;
  if(bleCmdQ.pop(msg)) handleBleCmd(msg);
  if(irCmdQ.pop(msg))  handleIrCmd(msg);
  if(rfCmdQ.pop(msg))  handleRfCmd(msg);
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length){
  ...
  if(t == TOPIC_MIBOX_CMD){
    ...
    ok = bleCmdQ.push(msg);
  } else if(t == TOPIC_IR_CMD){
    Serial.printf("[MQTT][IR] %s\n", msg.c_str());
    ok = irCmdQ.push(msg);
  } else if(t == TOPIC_433_CMD){
    ok = rfCmdQ.push(msg);
  }
  if(!ok) Serial.println("[MQTT] cmd queue full -> drop");
}

void setup(){
  Serial.begin(115200);
  delay(200);
  Serial.println("\n=== TSWell Universal Remote Gateway (gateway-fix) ===");
  ...
  Serial.println("[BLE] begin() + tx_power");
  irsend.begin();
  ...
  publish433Log("Universal Remote Booting (gateway-fix)");
}
...
void loop(){
  ...
  else if(mqttWasConnected && WiFi.status() == WL_CONNECTED){
    mqttWasConnected = false;
  ...
  bleHealthCheck();
  serviceCmdQueues();
  if(millis()-last433StatusMs>1500){ last433StatusMs=millis(); publish433Status(); }
  delay(1);
}
