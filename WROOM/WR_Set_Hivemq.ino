// WR_Set_Hivemq.ino
// ESP32 Web Sensor Dashboard + HiveMQ + AP 모드(BOOT 길게 눌러 전환)
...
/*
  ...
  - 시리얼 포맷:
    AHT:24.0°C 78% | SCD:28.9°C 53% CO2:1071ppm | LED:OFF | RSSI: -47 [dBm]
*/
...
// MQTT 토픽
const char* TOPIC_STATUS = "esp32wr/status";
const char* TOPIC_LED    = "esp32wr/led";
...
const int HIST_CAP = 120; // 120분
Record hist[HIST_CAP];
int histHead = 0;
int histCount = 0;
...
//  History
// ======================================================
void pushHistory(float at, float ah, float st, float sh, float co2, uint8_t led) {
  Record r;
  r.ts = millis();
  r.aht_t = at;
  r.aht_h = ah;
  ...
  if (histCount < HIST_CAP) histCount++;
}
...
void handleRoot(){
  addCORS();
  if (apMode) {
    // AP 모드에서는 설정 페이지 HTML 제공
    server.send_P(200, "text/html; charset=utf-8", PAGE_INDEX);
  } else {
    server.send(200, "text/plain; charset=utf-8",
                "ESP32 sensor board. Use external HTML (HiveMQ dashboard) or /api/status.");
  }
}
...
void handleApSave(){
  ...
  // TODO: 필요하면 NVS/Preferences 등에 저장하도록 확장
...
void handleHistory() {
  ...
  for (int i = 0; i < histCount; ++i) {
    if (i > 0) out += ",";
    int idx = (histHead - 1 - i + HIST_CAP) % HIST_CAP;
    const Record& r = hist[idx];
    out += "{";
    out += "\"ts\":"; out += String((unsigned long)r.ts);
    out += ",\"aht_t\":"; out += finite_f(r.aht_t) ? String(r.aht_t,1) : "null";
    out += ",\"aht_h\":"; out += finite_f(r.aht_h) ? String((int)round(r.aht_h)) : "null";
    out += ",\"scd_t\":"; out += finite_f(r.scd_t) ? String(r.scd_t,1) : "null";
    out += ",\"scd_h\":"; out += finite_f(r.scd_h) ? String((int)round(r.scd_h)) : "null";
    out += ",\"co2\":";   out += finite_f(r.co2)   ? String((int)round(r.co2)) : "null";
    out += ",\"led\":";   out += r.led ? "\"on\"" : "\"off\"";
    uint32_t dtMs = (nowMs >= r.ts) ? (nowMs - r.ts) : 0;
    out += ",\"age\":"; out += String((int)round(dtMs / 60000.0));
    out += "}";
  }
  out += "]";
...
void handleLedAny() {
  ...
  if (path == "/led/on" || path.endsWith("/led/on") ||
      path.endsWith("/api/led/on")) {
    on = true;
    hasCmd = true;
  } else if (path == "/led/off" || path.endsWith("/led/off") ||
             path.endsWith("/api/led/off")) {
    on = false;
    hasCmd = true;
...
void enterApMode() {
  ...
  bool ok = WiFi.softAP(apSsid);  // 비번 없음
...
void checkApButton() {
  ...
  if (!apMode) {
    if (level == LOW) {
      ...
      // 누르고 있는 동안 일정 간격으로 깜박이기 (AP 진입 전)
      ...
      // 5초 이상 계속 눌렸으면 AP 모드 진입
      if (held >= AP_HOLD_MS) {
        enterApMode();
      }
    } else {
      apPressStart = 0;
      // STA 모드에서는 LED를 ledState 상태로 유지
      digitalWrite(LED_PIN, ledState ? HIGH : LOW);
    }
  } else {
    // 이미 AP 모드이면 LED를 더 빠르게 계속 깜빡이기
...
void ensureMqtt() {
  if (apMode) return;                // AP 모드에서는 MQTT 시도 안 함
...
void setup() {
  ...
  server.on("/set",         HTTP_ANY,      handleLedAny);
  ...
  server.on("/api/led/on",  HTTP_ANY,      handleLedAny);
  server.on("/api/led/on",  HTTP_OPTIONS,  handleOptions);
  server.on("/api/led/off", HTTP_ANY,      handleLedAny);
  ...
  // MQTT (TLS, 인증 무시)
  espClient.setInsecure(); // 데모용, 인증서 검증 생략
...
void loop() {
  ...
  if (now - lastAvgMs >= 60000) {
    ...
    pushHistory(avg_aht_t, avg_aht_h, avg_scd_t, avg_scd_h, avg_co2, ledState ? 1 : 0);
